...
```{r spatiotemporal-modelling, eval=FALSE}
library(sdmTMB)
library(dplyr)
library(ggplot2)
library(scales)
...
#   dplyr::filter(survey_abbrev == "SYN WCVI")
...
```{r depth-time, eval=FALSE}
# d <- readRDS("../../gfs/report/data-cache/pacific-cod.rds")$survey_sets %>%
#   dplyr::filter(survey_abbrev == "SYN QCS")
# d <- mutate(d, Y = latitude, X = longitude)
dutm <- as_tibble(convert2utm(d))
dutm$density <- dutm$density_kgpm2 * 1000
dutm <- dplyr::filter(dutm, !is.na(depth_m))
dutm$depth_scaled <- scale(log(dutm$depth_m))
dutm$depth_scaled2 <- dutm$depth_scaled^2
pcod_spde <- make_spde(dutm$X, dutm$Y, n_knots = 250)
plot_spde(pcod_spde)
mtv <- sdmTMB(density ~ 0 + as.factor(year),
  time_varying = ~ 0 + depth_scaled + depth_scaled2,
  data = dutm, time = "year", spde = pcod_spde,
  family = tweedie(link = "log"), silent = FALSE)
print(mtv)
stopifnot(max(mtv$gradient) < 0.001)

nd <- data.frame(depth_scaled = seq(min(dutm$depth_scaled, na.rm = TRUE),
    max(dutm$depth_scaled, na.rm = TRUE), length.out = 100))
nd$depth_scaled2 <- nd$depth_scaled^2
get_tv_pred <- function(dat, year) {
  dat$year <- year
  p <- predict(mtv, newdata = dat, se_fit = TRUE, re_form = NA)
  p
}
future::plan(future::multisession)
out <- furrr::future_map(sort(unique(dutm$year)), 
  ~ get_tv_pred(dat = nd, year = .x))
future::plan(future::sequential)
out_df <- bind_rows(out)
.x <- log(dutm$depth_m)
out_df$depth <- exp(out_df$depth_scaled * sd(.x) + mean(.x))
ggplot(out_df, aes(depth, exp(est))) + 
  geom_ribbon(aes(
    ymin = exp(est - 2 * est_se), 
    ymax = exp(est + 2 * est_se), group = year), 
    fill = "grey60",  alpha  =0.15, colour = NA) +
  geom_line(aes(colour = as.factor(year)), lwd = .8) +
  # scale_colour_viridis_d() + 
  # scale_fill_viridis_d() +
  scale_colour_brewer(palette = "Dark2") + 
  scale_fill_brewer(palette = "Dark2") +
  gfplot::theme_pbs() +
  labs(colour = "Year", y = "Density", x = "Depth (m)") +
  coord_cartesian(expand = FALSE, xlim = c(min(out_df$depth), 450))
```
