...
Model convergence was informed by the statistics $\hat{R}$, the potential scale reduction statistic (should = 1) and $n_\mathrm{eff}$, which measures the effective number of MCMC independent samples after accounting for autocorrelation [@gelman1992; @gelman2014].
...
Posterior estimates of reference points and stock status conditioned on the 2020 catch levels from the model-averaged set are provided in Table \@ref(tab:tab-ref-points-table-avg-3cd).
...
Posterior estimates of reference points and stock status conditioned on the 2020 catch levels from the model-averaged set are provided in Table \@ref(tab:tab-ref-points-table-avg-5abcd).
