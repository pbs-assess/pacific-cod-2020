...
```{r tab-catch-3cd}
 ...
 cap = paste0("Reported catch (mt) of Pacific Cod in Area 3CD ",
              ...
              "observations and are considered to be more representative of true discarding. The 2020 total catch estimate is extrapolated from the total catch (landings plus discards) taken up to September 30, 2020 (see text). Therefore 2020 discards are not shown separately. Numbers are rounded for presentation."), french=french)
```
...
make.ref.points.table(c(base.model.3cd,
                        ...
                        caption = paste0("Posterior (2.5\\textsuperscript{th} percentile, Median, and ",
                                         ...
                                         "Biomass is in tonnes. All values are rounded. Ratios were calculated using full precision and therefore may not exactly match the numbers in the table."))
```
