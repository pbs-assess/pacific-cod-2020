...
dplyr::filter(dat$survey_index, survey_abbrev %in%
  ...
  knitr::kable(caption = "Données sur la morue du Pacifique, en tonnes métriques, tirées des relevés au chalut canadiens ( on ne tient pas compte de la capturabilité associée aux relevés). Les traits positifs représentent le nombre de traits de chalut dans lesquels des morues du Pacifique étaient présentes. Dans le tableau : OTHER HS MSA = autre relevé de l’assemblage de plusieurs espèces mené dans le détroit d’Hécate; SYN HS = relevé synoptique mené dans le détroit d’Hécate; SYN QCS = relevé synoptique mené dans le bassin de la Reine-Charlotte; SYN WCVI = relevé synoptique mené au large de la côte ouest de l’île de Vancouver.", booktabs = TRUE, linesep = "", 
...
make.parameters.est.table(base.model.5abcd,
  caption = paste0("Valeurs a posteriori (2,5\\textsuperscript{e} centile, médiane et ",
    "97,5 \\textsuperscript{e} centile) et estimations du mode de distribution a posteriori ",
    "pour les paramètres clés du scénario de référence de la zone 5ABCD. $R_0$ est en milliers de poissons. $\\hat{R}$ représente la statistique de réduction d’échelle potentielle et $n_\\mathrm{eff}$ correspond au nombre réel de tirages effectués lors de la simulation (voir le corps du texte). ", q.5abcd.desc),
  omit_pars = pars_est_omit, french=french)
```
