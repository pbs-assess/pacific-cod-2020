...
```{r surv-canadian, fig.cap="Données sur la morue du Pacifique provenant des relevés au chalut canadiens. La figure illustre les indices de la biomasse relative et les limites inférieures et supérieures des intervalles de confiance connexes. Les traits positifs représentent le nombre de traits de chalut dans lesquels des morues du Pacifique étaient présentes. Dans la figure : SYN QCS = relevé synoptique au chalut de fond mené dans le bassin de la Reine-Charlotte; SYN WCVI = relevé synoptique au chalut de fond mené au large de la côte ouest de l’île de Vancouver; SYN HS = relevé synoptique au chalut de fond mené dans le détroit d’Hécate; OTHER HS MSA = autre relevé de l’assemblage de plusieurs espèces mené dans le détroit d’Hécate."}
