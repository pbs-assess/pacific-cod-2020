# Plot spatial catch data, slight modification of gfplot::plot_cpue_spatial
# Essentially replace "cpue" with "catch",
#     and take sum not mean and convert to tonnes (at L303)

#' Plot a map of commercial Catch (currently TRAWL ONLY)
#'
#' @param dat Data from [get_catch_spatial()] #or [get_catch_spatial_ll()] (LL NOT DONE YET)
...
#'   year = 2013, catch = rlnorm(length(lat), log(1000), 0.6),
#'   vessel_registration_number = rep(seq_len(100), each = 10))
#' plot_catch_spatial(d, bin_width = 15, n_minimum_vessels = 1)
  ...
  function(dat,
    ...
    if (plot_hexagons) {
      ...
      g <- g + geom_polygon(data = public_dat, aes_string(
        x = "X", y = "Y",
        fill = "catch", colour = "catch", group = "hex_id"
...
enact_privacy_rule <- function(dat, bin_width, n_minimum_vessels, xlim, ylim) {
  ...
  # the actual catch hexagon binning:
  g <- ggplot(dat, aes_string("X", "Y")) +
    coord_equal(xlim = xlim, ylim = ylim) +
    stat_summary_hex(
      aes_string(x = "X", y = "Y", z = "catch"),
      data = dat, binwidth = bin_width,
      #fun = function(x) exp(mean(log(x), na.rm = FALSE))
      fun = function(x) sum(x/1000, na.rm = FALSE)
    )
  ...
  # Number of hexagon cells for vessel count and catch didn't match.
